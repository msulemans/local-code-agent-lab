...
def _controller_guidance(tool: str, *, patch_applied: bool, tests_passed: bool) -> str:
    ...
    if tool == "read_file":
        return "Evidence is available; propose an apply_patch next unless another concrete file is required."
    if tool == "apply_patch":
        return "Patch applied; run the registered tests next."
    if tool == "run_tests":
        return "Tests observed; revise with apply_patch if failing, otherwise review with git_diff."
    if tool == "git_diff":
