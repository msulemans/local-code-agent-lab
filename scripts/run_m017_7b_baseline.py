#!/usr/bin/env python3
"""Measure untouched pinned Qwen 7B on the strict executable patch gate."""
...
from localcode.patch_baseline import build_patch_messages, evaluate_patch_prediction, observe_failing_tests
...
def main() -> int:
    ...
    if Path(sys.prefix).resolve() != (ROOT / ".venv-mlx").resolve():
        raise SystemExit("M017 baseline must run with .venv-mlx/bin/python")
    ...
    if arguments.validate_only:
        print(json.dumps({
            "status": "m017_7b_baseline_ready", "run_id_available": not directory.exists(),
            "model_path": str(model_path), "registered": len(suite.cases),
...
def _summary(record: dict[str, object], directory: Path) -> dict[str, object]:
    return {
        ...
        "model_id": record["model_id"], "solved": record["solved"], "registered": record["registered"],
