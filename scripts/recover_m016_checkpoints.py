#!/usr/bin/env python3
"""Validate and executable-test checkpoints preserved by an interrupted M016 run."""
...
def main() -> int:
    ...
    if Path(sys.prefix).resolve() != (ROOT / ".venv-mlx").resolve():
        raise SystemExit("M016 recovery must run with .venv-mlx/bin/python")
    if (
        RUN_ID.fullmatch(arguments.source_run_id) is None
    ...
    record: dict[str, object] = {
        ...
        "state": "setup",
        "treatment": "shortened_completed_checkpoints_after_interrupted_training",
        "source_run_id": arguments.source_run_id,
        "source_run_state": None,
        "original_1600_step_treatment_complete": False,
        "available_checkpoint_iterations": [],
        "checkpoint_hashes": [],
    ...
    try:
        ...
        model_manifest = json.loads(
            (ROOT / "models/m015-local-model.json").read_text(encoding="utf-8")
        )
        _verify_inputs(config, base_config, model_manifest)
        suite = load_executable_suite(ROOT / config["executable_suite"], ROOT)

        source_record_path = ROOT / "runs/training" / arguments.source_run_id / "run.json"
        ...
        if source_record.get("state") != "failed" or source_record.get("sealed_examples_loaded") != 0:
            raise ValueError("source must be a failed zero-sealed M016 run")
        if source_record.get("experiment_id") != config["experiment_id"]:
            raise ValueError("source run does not belong to the frozen M016 experiment")
        ...
        if len(available) < 2:
            raise ValueError("recovery requires at least two completed checkpoints")
        record.update(
            state="checkpoint_selection_running",
            source_run_state=source_record["state"],
            available_checkpoint_iterations=list(available),
            checkpoint_hashes=[
                {
                    "iteration": iteration,
                    "sha256": _sha(source_adapter / f"{iteration:07d}_adapters.safetensors"),
                }
        ...
        # This result answers whether an available checkpoint is useful. It
        # deliberately does not relabel the interrupted 1,600-step treatment complete.
