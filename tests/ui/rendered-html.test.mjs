...
assert.match(html, /Controlled training ready/);
...
assert.match(js, /Matched ladder complete · 0\/3 throughout/);
assert.match(js, /gold 3\/3/);
assert.match(js, /B0\/A1\/A2\/A3 all 0\/3/);
assert.match(js, /valid patches 0\/1\/3\/2/);
assert.match(js, /reviewer protected a wrong core idea/i);
assert.match(js, /More valid patches still produced zero solves/);
assert.match(js, /Phase 4 contract passed/);
assert.match(js, /20 evaluation IDs \+ 20 revisions denied/);
assert.match(js, /Why split Phase 4 data by repair lineage/);
...
assert.match(js, /Executable baseline complete · 4\/6/);
assert.match(js, /validation loss 1\.383/);
assert.match(js, /executable base 4\/6/);
assert.match(js, /Why can zero solved cases still be a successful baseline run/);
assert.match(js, /id: "016"/);
assert.match(js, /Recovered negative · 1\/6/);
assert.match(js, /validation 1\.383 base → 1\.333 adapter/);
assert.match(js, /executable 4\/6 base → 1\/6 adapter/);
...
assert.match(js, /Executable LoRA negative · 0\/6/);
assert.match(js, /4,628 raw → 1,553 selected/);
assert.match(js, /468 bounded train \+ 80 validation/);
assert.match(js, /checkpoint 100 loss 1\.754/);
assert.match(js, /id: "017"/);
assert.match(js, /Qwen 7B selected · trusted edit 4\/6/);
assert.match(js, /strict diff 0\/6 · trusted edit 4\/6/);
