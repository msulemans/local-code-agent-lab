...
assert.match(js, /162 unit tests passed with 7 sandbox skips/);
assert.match(js, /Preflight blocks unsafe inference/);
assert.match(js, /atomic recorder preserves every CLI outcome/);
assert.match(js, /retained swap blocks Qwen before chat/);
assert.match(html, /007 guarded edit\/test loop/);
