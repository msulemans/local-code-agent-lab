...
class TrainingRunTests(unittest.TestCase):
    ...
    def test_training_metrics_drive_overfit_gate(self) -> None:
        output = (
            ...
            "Iter 40: Train loss 0.700, Learning Rate 1e-4, It/sec 1, "
            "Tokens/sec 110.0, Trained Tokens 4000, Peak mem 4.200 GB\n"
        )
        metrics = parse_train_metrics(output)
        self.assertEqual([metric.iteration for metric in metrics], [5, 40])
        self.assertTrue(diagnostic_passed(metrics, required_relative_improvement=0.1, maximum_peak_memory_gb=24))
        self.assertFalse(diagnostic_passed(metrics, required_relative_improvement=0.4, maximum_peak_memory_gb=24))
